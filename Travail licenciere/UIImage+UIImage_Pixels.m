...
+(UIImage *)getImageWithGrayScalePixels:(unsigned char *)inputVector width:(int)width height:(int)height
{
    #define BITS_PER_PIXEL 8
    #define BITS_PER_COMPONENT (BITS_PER_PIXEL)
    #define BYTES_PER_PIXEL (BITS_PER_PIXEL/BITS_PER_COMPONENT)
	...
	CGContextRef context = CGBitmapContextCreate(inputVector, width, height, BITS_PER_COMPONENT, bytesPerRow, colourSpace, kCGImageAlphaNone);
...
{
    unsigned char* pixelData = [self grayscalePixels];
...
+(UIImage *)imageOfHyerogliphWithMainKey:(int)mainKey pattern:(KeyPosition)pattern number:(int)number
{
    return [UIImage imageNamed:[NSString stringWithFormat:@"[%@]%i_%i", [HyerogliphPatternHelper getStringNameOfPatternForPatternNumber:pattern], mainKey, number]];
}

+(UIImage *)imageOfKeyWithNumber:(int)keyNumber pattern:(KeyPosition)pattern
{
    return [UIImage imageNamed:[NSString stringWithFormat:@"[%@]%ik_%i", [HyerogliphPatternHelper getStringNameOfPatternForPatternNumber:pattern], keyNumber, 0]];
}
